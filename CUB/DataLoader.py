...
def load_test_bbox(
    ...
    resize_size,
    crop_size,
    test_txt_path='CUB_DATA/test_list.txt',
    test_gt_path='CUB_DATA/test_bounding_box.txt',
):
    test_gt = []
    test_txt = []
    shift_size = (resize_size - crop_size) // 2.0
    ...
    with open(test_gt_path, 'r') as f:
        ...
        for line in f:
            ...
            x0, y0, x1, y1 = (
                x0 / w * resize_size - shift_size,
                y0 / h * resize_size - shift_size,
                x1 / w * resize_size - shift_size,
                y1 / h * resize_size - shift_size,
            )
            if x0 < 0:
                x0 = 0
            if y0 < 0:
