...
def load_test_bbox(root, crop_size, resize_size, test_gt_path='val_gt.txt'):
    test_gt = []
    test_txt = []
    ...
    with open(test_gt_path, 'r') as f:
        ...
        for line in f:
            ...
            for i in range(box_num):
                bbox[4 * i], bbox[4 * i + 1], bbox[4 * i + 2], bbox[4 * i + 3] = (
                    bbox[4 * i] / w * resize_size - shift_size,
                    bbox[4 * i + 1] / h * resize_size - shift_size,
                    bbox[4 * i + 2] / w * resize_size - shift_size,
                    bbox[4 * i + 3] / h * resize_size - shift_size,
                )
                if bbox[4 * i] < 0:
                    bbox[4 * i] = 0
                if bbox[4 * i + 1] < 0:
...
class ImageDataset(data.Dataset):
    def __init__(self, args):
        ...
        if self.phase == 'train':
            ...
            self.test_bbox = load_test_bbox(self.root, self.crop_size, self.resize_size)
