...
class Model(nn.Module):
    ...
    def forward(self, x, label=None, N=1):
        ...
        if self.args.phase == 'train':
            losses = self.get_AllLoss(
                label=label, score_1=self.score_1, score_2=self.score_2
            )
            return {'score_1': self.score_1, 'score_2': self.score_2, 'losses': losses}
    ...
    def get_AllLoss(self, label, score_1, score_2):
        loss_func = nn.CrossEntropyLoss().cuda()
        loss_1 = loss_func(score_1, label).cuda()
        loss_2 = loss_func(score_2, label).cuda()
        loss_3 = self.active_loss(label=label, min_area=0.05, max_area=0.3).cuda()
        loss_4 = self.ac_loss().cuda()
        loss_5 = self.bf_loss().cuda()
    ...
    def weight_deepcopy(self):
        ...
        for i in range(len(self.classifier_cls)):
            if 'Conv' in str(self.classifier_cls[i]) or 'BatchNorm2d' in str(
                self.classifier_cls[i]
            ):
                self.classifier_cls_copy[i].weight.data = (
                    self.classifier_cls[i].weight.clone().detach()
                )
                self.classifier_cls_copy[i].bias.data = (
                    self.classifier_cls[i].bias.clone().detach()
                )
    ...
    def active_loss(self, label=None, min_area=0, max_area=1.0):
        ...
        for i in range(batch_size):
            ...
            if (bg_probs[i][label[i]] > fg_probs[i][label[i]]) and (
                self.args.lambda_e != 0.0
            ):
